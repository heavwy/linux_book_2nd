...
ledpwm {
		...
		bp1 {
			label = "KEY_1";
			gpios = <&gpioa 13 GPIO_ACTIVE_LOW>; // B4
			trigger = "falling";
		};
		
		bp2 {
			label = "KEY_2";
			gpios = <&gpioa 14 GPIO_ACTIVE_LOW>; // B3
			trigger = "falling";
		};
...
{	
	...
	device_for_each_child_node(dev, child){
		...
		if (strcmp(label_name,"led") == 0) {
			...
			/* set led state to off. In the DT GPIO_ACTIVE_LOW means that 1 in gpiod_set_value() means 0 in the gpio output */
